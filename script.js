...
function showMeme() {
  ...
  memeImg.setAttribute('src',randomMemeUrl);
  clearAll();
  currentContainer.append(memeImg);
}
...
function showQuote() {
  ...
  currentContainer = containers[2];

  quote.textContent = "- "+randomQuote.quote+ " author: "+randomQuote.author;
  clearAll();
...
function revealAnswers() {
  if (currentContainer === containers[3] && !riddleAnswer.hidden)
  {
    alert('The answer is already revealed...');
  } else if(currentContainer === containers[3])
  {
    riddleAnswer.hidden = false;
  } else if (currentContainer !== containers[3])
  {
    alert('You must generate riddle first!')
  }
